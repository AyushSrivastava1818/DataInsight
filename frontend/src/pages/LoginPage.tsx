...
import { LogIn, Sparkles, AlertCircle, Globe } from "lucide-react";
export const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!supabase) {
      setError(
        "Auth services are currently offline. Check environment settings.",
      );
      return;
    }
    setError(null);
      ...
      <div className="w-full max-w-md space-y-6 fade-in">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-slate-400 text-sm">
            Deploy premium statistical cleaning & profiling workstation
          </p>
        </div>
        ...
        <GlassCard className="p-8 border-white/5 bg-slate-950/40 backdrop-blur-md">
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-slate-950/10 px-3 text-slate-500 font-bold">
                Or continue with
              </span>
            </div>
          </div>
          ...
          >
            <Globe className="w-4 h-4 text-rose-500" /> Sign in with Google
          </button>
