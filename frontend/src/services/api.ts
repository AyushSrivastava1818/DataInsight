...
export interface EDASummary {
  ...
  categorical_summary: Record<string, { unique_count: number; top_categories: Record<string, number> }>;
  correlation: {
    matrix: Record<string, Record<string, number>>;
    strong_relationships: Array<{ column1: string; column2: string; coefficient: number; direction: 'positive' | 'negative' }>;
  };
...
export const api = {
  ...
  async getPreview(id: number, version?: number): Promise<PreviewData> {
    ...
    const headers = await getHeaders();
    const response = await fetch(`${API_BASE_URL}/datasets/${id}/preview${query}`, { headers });
    if (!response.ok) throw new Error('Failed to retrieve preview.');
    return response.json();
  },

  // Cleaning Endpoints
  async imputeMissing(id: number, column: string, strategy: string): Promise<DatasetVersion> {
    const headers = await getHeaders();
    const response = await fetch(`${API_BASE_URL}/datasets/${id}/clean/missing-values`, {
      method: 'POST',
      headers,
  ...
  async removeDuplicates(id: number): Promise<DatasetVersion> {
    const headers = await getHeaders();
    const response = await fetch(`${API_BASE_URL}/datasets/${id}/clean/duplicates`, {
      method: 'POST',
      headers,
    });
    if (!response.ok) {
  ...
  async typeCorrection(id: number, column: string, targetType: string, dateFormat?: string): Promise<DatasetVersion> {
    const headers = await getHeaders();
    const response = await fetch(`${API_BASE_URL}/datasets/${id}/clean/type-correction`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ column, target_type: targetType, date_format: dateFormat }),
    });
    if (!response.ok) {
      const err = await response.json();
  ...
  async handleOutliers(id: number, column: string, method: string, strategy: string, threshold = 3.0): Promise<DatasetVersion> {
    const headers = await getHeaders();
    const response = await fetch(`${API_BASE_URL}/datasets/${id}/clean/outliers`, {
      method: 'POST',
      headers,
  ...
  async cleanText(id: number, column: string, options: { remove_extra_spaces: boolean; casing?: string; remove_special_chars: boolean }): Promise<DatasetVersion> {
    const headers = await getHeaders();
  ...
  async columnOp(id: number, action: 'rename' | 'delete' | 'create', column: string, newName?: string, expression?: string): Promise<DatasetVersion> {
    const headers = await getHeaders();
    const response = await fetch(`${API_BASE_URL}/datasets/${id}/clean/column-op`, {
      method: 'POST',
      headers,
  ...
  async getEDA(id: number, version?: number): Promise<EDASummary> {
    const query = version ? `?version=${version}` : '';
    const headers = await getHeaders();
    const response = await fetch(`${API_BASE_URL}/datasets/${id}/eda${query}`, { headers });
  ...
  async getInsights(id: number, version?: number): Promise<Insight[]> {
    const query = version ? `?version=${version}` : '';
    const headers = await getHeaders();
    const response = await fetch(`${API_BASE_URL}/datasets/${id}/insights${query}`, { headers });
  ...
  async getChatHistory(id: number): Promise<ChatMessage[]> {
    const headers = await getHeaders();
    const response = await fetch(`${API_BASE_URL}/datasets/${id}/chat`, { headers });
